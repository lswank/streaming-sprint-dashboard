...
for a in market lease-terms exit-cost; do
  nohup grok --prompt-file "RUNDIR/prompts/$a.md" --always-approve \
    --cwd RUNDIR > "RUNDIR/state/$a.stdout" 2>&1 &
done
```

`--always-approve` is required: without it each agent stops at the first tool
approval prompt with nobody watching, and its card sits queued forever. Pass
`--model` with a cheaper model for the legwork and keep the expensive one for
verification. Every launched process gets its own stdout file: when a card stays
queued, that file says why.
